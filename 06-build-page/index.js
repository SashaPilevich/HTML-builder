...
async function copyStyles() {
  const allFilesOfStyle = await fs.readdir(pathToStyles, {
    withFileTypes: true,
  });
  ...
  try {
    ...
    (await filesForCopy).forEach(async (item) => {
      if (item.isDirectory) {
        ...
        const fileForCopy = await fs.readdir(
          path.join(pathToAssets, item.name)
        );
        fileForCopy.forEach(async (file) => {
          await fs.copyFile(
            path.join(pathToAssets, item.name, file),
            path.join(pathToCopyAssets, item.name, file)
          );
        });
      }
    });
...
async function createProject() {
  ...
  const mainIndex = createWriteStream(
    path.join(pathToProjectFolder, "index.html")
  );
  createHtml();
  copyStyles();
